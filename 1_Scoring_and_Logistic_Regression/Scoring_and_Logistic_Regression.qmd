...
The case study by Desbois will be partly reproduced in the next lesson.
...
and knowing $\pi_k = \mathbb{P}[Y=k]$ we have:

$$\mathbb{P}[Y=k|X=x]=\frac{f_k(x)\pi_k}{\mathbb{P}[X=x]}$$
Rewriting Bayes Classifier as: 
$$
f^*(x)=\left\{ \begin{array}{ll} 
...
Or taking the log:
$$\log(\frac{f_1(x)\pi_1}{f_0(x)\pi_0})\geq 0$$
We have:
$$\log(f_k(x)\pi_k)=\log f_k(x) + \log \pi_k =C -\frac{1}{2} \log |\Sigma_k| -\frac{1}{2} (x-\mu_k)^T \Sigma_k^{-1} (x-\mu_k)+\log \pi_k $$
Usually discriminant functions $\delta_k$ are defined as:
$$\delta_k(x)=-\frac{1}{2} \log |\Sigma_k| -\frac{1}{2} (x-\mu_k)^T \Sigma_k^{-1} (x-\mu_k)+\log \pi_k $$
So the Bayes Classifier rewrites:
$$
f^*(x)=\left\{ \begin{array}{ll} 
