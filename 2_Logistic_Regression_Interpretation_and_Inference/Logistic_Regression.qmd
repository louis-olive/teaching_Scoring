...
```
We visualize the farm holdings as a bivariate plot on the first two components  of PCA (based on financial ratios), we use illustrative variable Y (0=”healthy”; 1=”failing”):
```{r}
#| code-fold: true
...
`glmnet` also provides an automatic and efficient procedure to select $\lambda$ implementing K-fold Cross-Validation (see next question for implementation of the method in general):
