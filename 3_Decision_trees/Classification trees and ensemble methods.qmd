...
\end{array} \right.
$$
Here the authors parameterize the binomial probabilities by:

$p(x)=\frac{e^{f(x)}}{e^{f(x)}+e^{-f(x)}}=\frac{1}{1+e^{-2f(x)}}$

and introduce $Y' = \frac{Y+1}{2} \in \{0, 1\}$ for $Y \in \{-1, 1\}$.
...
It is a Forward Stagewise Additive Method, at each iteration a Newton step is perform, in the context of Decision Trees, this step is equivalent to fitting a Regression Tree on weighted residuals.

We check our implementation against `xgboost`
...
We show LogitBoost decision boundary, here after $100$ iterations using our version of the algorithm:
...
Here after $20$ iterations using `xgboost`:
...
TODO comparing for various values of shrink / learning rate parameter
...
We then investigate a more recent version of LogitBoost algorithm, allowing to close the gap between the original @fht2000 and the more recent and state of the art `xgboost` @xgboost2016.
...
The underlying idea common to both Robust LogitBoost and `xgboost` is to take advantage of some simplifications that occur for Gradient Boosting / Newton methods when base learners are Decision Trees. In particular it is possible to show that the problem is equivalent to fitting a Decision Tree using a novel splitting formula, more precisely using a Sum of Squares criterion (Regression tree) involving the gradient and the hessian of any convex loss.

In the @hastie2009, authors suggest that AdaBoost or any comparable FSAM can be fitted using specialized trees (ie specific gain used to split), but prefer to follow the approach of Friedman's Gradient Boosting:
![](images/ESL_specialized_trees.png){fig-align="center"}


We implement here the Ping Li Robust LogitBoost method, re-using our implementation of a Decision Tree at the start of the course.

First defining specialized `xgboost` / Robust LogitBoost gains (using Gradient, Hessian, $\lambda$):
```{r}
#| code-fold: show
...
Then slightly modifying the other functions to adapt the problem to a regression (the implementation at the start of the lesson was dedicated to binary classification problem):
...
Our toy implementation of Robust Logitboost agrees with xgboost in terms of base learners fitted:

Checking for depth 1 weak learners:
